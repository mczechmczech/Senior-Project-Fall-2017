...
public class SQLQueryBuilder {
	...
	void addUser(String user, String password, String first, String last, boolean admin) {
		try(Connection connection = ConnectionPool.getConnection()) {
			...
			connection.close();
		} catch (SQLException e) { throw new IllegalStateException("Cannot register user!", e); }
	}
	...
	void addTask(int ID, boolean isNew)	{
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			connection.close();
		} catch (Exception e) { throw new IllegalStateException("Cannot add task!", e); }
	}
	...
	void editTask(int taskIDNum) {
		...
		catch (Exception e) { throw new IllegalStateException("Cannot edit task!", e); }
	}
	...
	void taskAccepted(int taskIDNum) {
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			connection.close();
		} catch (Exception e) { throw new IllegalStateException("Cannot accept task!", e); }
	}
	...
	void putInTrash(int taskIDNum) {
		...
		catch (Exception e) { throw new IllegalStateException("Cannot put task in trash!", e); }
	}
	...
	void deleteFromTrash(int taskIDNum) {
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			connection.close();
		} catch (Exception e) { throw new IllegalStateException("Cannot delete task from trash!", e); }
	}
	...
	void retrieveFromTrash(int taskIDNum) {
		try(Connection connection = ConnectionPool.getConnection()) {
			...
			connection.close();
		} catch (Exception e) { throw new IllegalStateException("Cannot retrieve task from trash!", e); }
	}
	 ...
	 * 				TODO: add rest of possible parameters
	...
	ArrayList<Task> getTasks(int ID, String table, String search) {
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			query = 
				table.equals("user") ?
					"SELECT * FROM TASK WHERE user_assigned_ID = " + ID  + " AND is_complete = 0" + " AND is_new = 0" + " AND is_trash = 0" :
				table.equals("all") ?			
					"SELECT * FROM TASK"  + " WHERE is_complete = 0" + " AND is_new = 0" + " AND is_trash = 0" :			
				table.equals("inboxTasks") ?			
					"SELECT * FROM TASK WHERE user_assigned_ID = '" + ID + "' AND is_new = 1" + " AND is_complete = 0" + " AND is_trash = 0" :			
				table.equals("sentTasks") ?			
					"SELECT * FROM TASK WHERE user_created_ID = '" + ID + "' AND is_complete = 0" + " AND is_trash = 0" + " AND user_assigned_ID != '" + ID + "'" :			
				table.equals("archive") ?			
					"SELECT * FROM TASK WHERE user_assigned_ID = '" + ID + "' AND is_complete = 1" + " AND is_new = 0" + " AND is_trash = 0" :			
				table.equals("allArchive") ?			
					"SELECT * FROM TASK WHERE is_complete = 1" + " AND is_new = 0" + " AND is_trash = 0" :			
				table.equals("trashReceivedTasks") ?			
					"SELECT * FROM TASK WHERE user_assigned_ID = '" + ID + "' AND is_trash = 1" + " AND is_new = 0" :			
				table.equals("trashSentTasks") ?			
					"SELECT * FROM TASK WHERE user_created_ID = '" + ID + "' AND is_trash = 1" + " AND is_new = 0" :
				table.equals("created") ?			
					"SELECT * FROM TASK WHERE user_created_ID = '" + ID + "' AND is_trash = 0" : null;			
			
			query += 
				search.equals("") ? "" : " AND ((task_name LIKE '%"+search+"%') OR (due_date LIKE '%"+search+"%') OR (task_descr LIKE '%"+search+"%') OR (task_notes LIKE '%"+search+"%'))";
			...
			while (srs.next()) {
				...
				if (!(Integer.parseInt(task.getProjectNum()) == 0)) { tasks.add(task); }
			}
			
			connection.close();
			return tasks;
		}
		catch (Exception e) { throw new IllegalStateException("Cannot get tasks!", e); }
	}
	...
	ArrayList<Task> getSubTasks(int taskID)
	{
		...
		catch (Exception e) { throw new IllegalStateException("Cannot get subtasks!", e); }
	}
	...
	void newMessage(int receiverID, String message, int senderID) {
		...
		catch (Exception e) { throw new IllegalStateException("Cannot create new message!", e); }
	}
	 ...
	 * 				sentMessages - Updates the table of messages sent by the logged in user
	...
	ArrayList<Message> getMessages(int ID, String table) {
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			query = 
				table.equals("inboxMessages") ?
					"SELECT * FROM MESSAGE WHERE user_received_ID = '" + ID + "' AND user_received_is_trash = 0" + " AND user_received_remove_trash = 0" :
				table.equals("sentMessages") ?			
					"SELECT * FROM MESSAGE WHERE user_created_ID = '" + ID + "' AND user_created_is_trash = 0" + " AND user_created_remove_trash = 0" :
				table.equals("inboxMessagesTrash") ?
					"SELECT * FROM MESSAGE WHERE user_received_ID = '" + ID + "' AND user_received_is_trash = 1" + " AND user_received_remove_trash = 0" :
				table.equals("sentMessagesTrash") ?			
					"SELECT * FROM MESSAGE WHERE user_created_ID = '" + ID + "' AND user_created_is_trash = 1" + " AND user_created_remove_trash = 0" : null;
			...
			connection.close();
			return messages;
		} catch (Exception e) { throw new IllegalStateException("Cannot get messages!", e); }
	}
	...
	void putMessageInTrash(int messageID, String table)	{
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			query = 
				...
				table.equals("sentMessages") ?			
					"UPDATE senior.MESSAGE SET user_created_is_trash = 1 WHERE message_ID = " + messageID : null;
			PreparedStatement s = connection.prepareStatement(query);
			s.execute();
			
			connection.close();
		} catch (Exception e) { throw new IllegalStateException("Cannot put message in trash!", e); }
	}
	...
	void removeMessageFromTrash(int messageID, String table) {
		try(Connection connection = ConnectionPool.getConnection()) {
			...
			query = 
				...
				table.equals("sentMessages") ? 
						"UPDATE senior.MESSAGE SET user_created_remove_trash = 1 WHERE message_ID = " + messageID : null;
			PreparedStatement s = connection.prepareStatement(query);
			s.execute();
			...
			s2.execute();
			connection.close();
		} catch (Exception e) { throw new IllegalStateException("Cannot remove message from trash!", e); }
	}
	...
	void addCategory(String cat) {
		...
		{			
			...
			s.execute();			
			connection.close();
		} catch (Exception e) {	throw new IllegalStateException("Cannot add category!", e); }
	}
	...
	ArrayList<String> getCategories() {
 		try(Connection connection = ConnectionPool.getConnection()) {
 			...
 			while(srs.next()) { categories.add(srs.getString("category")); }
 			connection.close();
 			return categories; 			
 		} catch (SQLException e) { throw new IllegalStateException("Cannot get categories!", e); } 
 	}
	...
	boolean containsCategory(String cat) {
 		try(Connection connection = ConnectionPool.getConnection()) {
 			...
 			PreparedStatement s = connection.prepareStatement(query);
 			ResultSet srs = s.executeQuery();
 			while(srs.next()) {	if(cat.equals(srs.getString("category"))) return true; }
 			connection.close();
 			return false; 			
 		} catch (SQLException e) { throw new IllegalStateException("Cannot compare categories!", e); } 
 	}
	...
	ArrayList<String> getUsers() {
 		try(Connection connection = ConnectionPool.getConnection()) {
 			...
 			PreparedStatement s = connection.prepareStatement(query);
 			ResultSet srs = s.executeQuery();
 			while(srs.next()) { users.add(srs.getString("username")); }
 			connection.close();
 			return users;
 		} catch (SQLException e) { throw new IllegalStateException("Cannot get users!", e); } 
	 }
	...
	int getUserCreatedID(int taskID) {
		...
		try(Connection connection = ConnectionPool.getConnection())
		{
			...
			PreparedStatement s = connection.prepareStatement(query);
			ResultSet srs = s.executeQuery(query);
			if(srs.next()) { userCreated = srs.getInt("user_created_ID"); }
			connection.close();
			return userCreated;
		} catch (Exception e) {	throw new IllegalStateException("Cannot get UserCreatedID!", e); }
	}
	...
	String getUserNameFromID(int ID) {
		...
		try(Connection connection = ConnectionPool.getConnection())
		{
			...
			if(srs.next()) { nameOfUser = srs.getString("username"); }
			connection.close();
			return nameOfUser;
		} catch (Exception e) { throw new IllegalStateException("Cannot get username from ID!", e); }
	}
	...
	int getIDFromUserName(String nameOfUser) {
		...
		try(Connection connection = ConnectionPool.getConnection())	{
			...
			while(srs.next()) {	if (srs.getString("username").equals(nameOfUser)) ID = srs.getInt("user_ID"); }
			connection.close();
			return ID;
		} catch (Exception e) { throw new IllegalStateException("Cannot get ID from username!", e); }
	}
	...
	boolean checkPassword(String nameOfUser, char[] passwordOfUser) {
		...
		} catch (SQLException e) { throw new IllegalStateException("Cannot check password!", e); } 
	}
}
